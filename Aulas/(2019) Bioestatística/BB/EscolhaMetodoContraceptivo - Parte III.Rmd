---
title: "Escolha do Método Contraceptivo"
...
1. Será que a religião influencia no número de filhos?
2. O padrão de vida está relacionado com o uso de métodos contraceptivos?
3. Mulheres com mais idade possuem um maior número de filhos? 
...
## Religião (Qualitativa) x N. filhos (Quantitativa)
...
ggplot(dados) +
 aes(x = NumeroFilhos, fill = ReligiaoEsposa) +
 geom_density(adjust = 1L) +
 scale_fill_hue() +
 theme_minimal()
```

## Padrão de vida (Qualitativa) x Métodos Contraceptivos (Qualitativa)

```{r, message=FALSE,warning=FALSE}
library(summarytools)

ctable(dados$PadraoVida, dados$MetodoContraceptivo, prop = "r") # perfil linha
ctable(dados$PadraoVida, dados$MetodoContraceptivo, prop = "c") # perfil coluna

# Substituindo os níveis da variável MetodoContraceptivo para apenas Usa e Não Usa
levels(dados$MetodoContraceptivo) <- c("NãoUsa", "Usa", "Usa")

# Avaliando o teste qui-quadrado
ctable(dados$PadraoVida, dados$MetodoContraceptivo, prop = "r", chisq = T)

library(ggplot2)

ggplot(dados) +
 aes(x = PadraoVida, fill = MetodoContraceptivo) +
 geom_bar(position = "fill") +
 scale_fill_hue() +
 theme_minimal()
```

## Idade (Quantitativa) x N. de filhos (Quantitativa)

```{r, message=FALSE,warning=FALSE}
library(ggplot2)

ggplot(dados) +
 aes(x = IdadeEsposa, y = NumeroFilhos) +
 geom_point(size = 1L, colour = "#0c4c8a") +
 geom_smooth(span = 0.75) +
 theme_minimal()

cor(dados$IdadeEsposa,dados$NumeroFilhos)
```

## Religião (Qualitativa) x Escolaridade (Qualitativa)

```{r, message=FALSE,warning=FALSE}
library(ggplot2)

ggplot(dados) +
 aes(x = EducacaoEsposa, fill = ReligiaoEsposa) +
 geom_bar(position = "fill") +
 scale_fill_hue() +
 theme_minimal()

ctable(dados$EducacaoEsposa, dados$ReligiaoEsposa, chisq = T)
```

## Exposição à mídia (Qualitativa) x Métodos contraceptivos (Qualitativa)

```{r, message=FALSE,warning=FALSE}
library(ggplot2)

ggplot(dados) +
 aes(x = ExposicaoMidia, fill = MetodoContraceptivo) +
 geom_bar(position = "fill") +
 scale_fill_hue() +
 theme_minimal()

ctable(dados$ExposicaoMidia, dados$MetodoContraceptivo, chisq = T)
```
